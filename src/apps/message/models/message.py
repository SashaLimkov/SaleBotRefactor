...
class Message(TimeBasedModel):
    ...
    name = models.CharField(
        "Название", max_length=60, unique=True
    )  # Поле используемое для получения сообщений из бота
...
class MessageContent(TimeBasedModel):
    ...
    message = models.ForeignKey(
        to=Message, on_delete=models.SET_NULL, null=True, verbose_name="Сообщение"
    )
