...
class SettingsUser(TimeBasedModel):
    ...
    CURRENCY_CHOICES = ((SHOP_CURRENCY, "Валюта магазина"), (RUB, "Рубли"))
    ...
    currency = models.IntegerField(
        "Валюта", choices=CURRENCY_CHOICES, default=SHOP_CURRENCY
    )
    formula = models.IntegerField(
        "Формула ценообразования", choices=FORMULA_CHOICES, default=NO_COMMISSION
    )
    commission = models.FloatField("Комиссия", blank=True, null=True, default=0.0)
    rounder = models.IntegerField(
        "Уровень округления", default=0, blank=True, null=True
    )

    logo = models.ImageField(verbose_name="Логотип", blank=True, null=True)
    text_logo = models.CharField(
        verbose_name="Текстовый Логотип", max_length=255, blank=True, null=True
    )
