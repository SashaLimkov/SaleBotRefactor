...
async def formula_and_commission_menu(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if user_action == 0:
        ...
        text = get_message_by_name_for_user(name="enter_commission", telegram_id=user_id).text
...
async def get_formula_commission_text_and_selected_formula(user_id: int) -> tuple:
    ...
    text = get_message_by_name_for_user(name="formula_with_or_without_comission", telegram_id=user_id).text.format(
