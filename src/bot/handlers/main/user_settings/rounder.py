...
async def select_rounder_lvl(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    update_field_settings(
        telegram_id=user_id,
        field="rounder",
        value=selected_round_lvl
    )
    call_text = get_message_by_name_for_user(
        name="select_round_lvl",
        telegram_id=user_id).text.format(selected_rounder=selected_round_lvl)
    await call.answer(text=call_text, show_alert=True)
    text = get_message_by_name_for_user(name="rounder_settings").text.format(selected_rounder=selected_round_lvl)
