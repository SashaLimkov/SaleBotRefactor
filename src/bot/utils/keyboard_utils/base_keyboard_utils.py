...
async def get_base_keyboard(buttons: list,
    ...
    """
    keyboard = await get_keyboard_markup(keyboard_options=keyboard_options, is_inline=is_inline)
...
async def get_keyboard_markup(
        keyboard_options: dict,
        is_inline: bool) -> types.InlineKeyboardMarkup | types.ReplyKeyboardMarkup:
    """
    :param keyboard_options: keyboard_options
    :param is_inline: bool argument for creating inline or reply keyboard markup
    :return: keyboard markup obj
    """
    return types.InlineKeyboardMarkup(**keyboard_options) if is_inline else types.ReplyKeyboardMarkup(
        **keyboard_options
    )


async def get_keyboard_button(button: dict, is_inline: bool) -> types.InlineKeyboardButton | types.KeyboardButton:
    """
    :param button: button options
    :param is_inline: bool argument for creating inline or reply button
    :return: keyboard button obj
    """
    return types.InlineKeyboardButton(**button) if is_inline else types.KeyboardButton(**button)
