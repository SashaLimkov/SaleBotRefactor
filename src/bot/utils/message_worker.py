...
async def try_edit_message(message, user_id, text, main_message_id, keyboard, state: FSMContext):
    try:
...
async def try_send_message(message, user_id, text, keyboard, state: FSMContext):
    ...
    try:
        ...
        await notice_programmers(exception_info=traceback.format_exc(), **message.chat.to_python())


async def try_send_video(video_path, chat_id: int, text, message, state: FSMContext, keyboard,
